...
<script>
  ...
  export default {
    ...
    created(){
      if(this.isEdit){
        getProduct(this.$route.query.id).then(response=>{
          this.productParam = {
            ...response.data,
            pic: response.data.pic ? JSON.parse(response.data.pic) : []
          };
        });
      }
    },
    methods: {
      getPicUrl(url) {
        this.productParam.pic.push(url);
      },
      hideAll() {
      ...
      finishCommit(isEdit) {
        this.$confirm('是否要提交该产品', '提示', {
          ...
          if(isEdit){
            updateProduct(this.$route.query.id,{
              ...this.productParam,
              pic: JSON.stringify(this.productParam.pic)
            }).then(response=>{
              this.$message({
                type: 'success',
                message: '提交成功',
