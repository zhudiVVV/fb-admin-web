  ...
  <el-card class="form-container" shadow="never">
    <el-form :model="productCate"
      ...
      <el-form-item label="分类图标：">
        <single-upload v-model="productCate.icon" @success="getProductCateIcon"></single-upload>
      </el-form-item>
...
<script>
  import {productCategoryList, createProductCate, updateProductCate, getProductCate} from '@/api/productCate';
  ...
  export default {
    ...
    async mounted() {
      if (this.isEdit) {
        var p = await getProductCate(this.$route.query.id)
            // .then(response => {
          console.log('================================')
          console.dir(p)
          this.productCate = p.data;

        // });
        getProductAttrInfo(this.$route.query.id).then(response => {
          console.log('getProductAttrInfo================================>', response)
      ...
      this.getSelectProductCateList();
        await this.getProductAttrCateList();
    },
    methods: {
      getProductCateIcon(data) {
        this.productCate.icon = data
      },
      getSelectProductCateList() {
        productCategoryList(0, {pageSize: 100, pageNum: 1}).then(response => {
          this.selectProductCateList = response.data.content || [];
          this.selectProductCateList.unshift({id: 0, name: '无上级分类'});

            // this.$nextTick(() => {
                console.log('parentId=======>', Number(this.$route.query.parentId))
                this.productCate.parentId = Number(this.$route.query.parentId) || 0
            this.parentIdDisabled = Number(this.$route.query.parentId) > 0
            // })
        });
      },
      getProductAttrCateList() {
        fetchListWithAttr().then(response => {
          console.log('getProductAttrCateList================================>', response)
